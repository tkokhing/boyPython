...
# # source https://github.com/google/certificate-transparency
...
#     def test_get_response_one_retry(self, site):
...
#                           handler.get_response, (site))
# def use_googleCT(site):
#     try:
#         test_get_response_one_retry(site)
#     # except requests.exceptions.SSLError as error:
#     except Exception as e:
#         # print("doesn't have SSL working properly (%s)" % (error, ))
#         return "Site error"

#     if '200' in canned_response.status_code:
#         # print("yes")
#         return "Yes"
#     else:
#         # print("no")
...
def main():
    ...
    appFile = open('WebsiteCheck_200v4.txt','a')
    dataFmt = "{:^5}  {:<35} {:<15} {:<10}\n"
    appFile.write('S/No   Website                            HSTS (Y/N)     CT (Y/N)       \n')
    sourcepath = 'D:/kht/10_MSSD/10.4_T2/51.502 SS/L5ss/top-1m.csv'
    ...
    for i in range (1,201):
        siteNum = random.randint(0, 935760)
        site = df.iloc[siteNum,1]
        ans_hsts = has_hsts(site)
        # ans_CT = use_googleCT(site)
        appFile.write(dataFmt.format(str(i), site, ans_hsts, ans_hsts))
